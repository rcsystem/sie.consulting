...
import AttendanceExportDrawer from "../../components/Attendance/AttendanceExportDrawer";
...
function procesarSemana(rawData: any[], diasSemana: string[]): EmployeeWeek[] {
  ...
  for (const r of rawData) {
    ...
    if (diasSemana.includes(fecha)) {
      ...
      if (["present", "late"].includes(r.attendance_status))
        emp.total_present++;
      if (r.is_late) emp.total_late++;
      if (
        ["unjustified_absence", "justified_absence", "absent"].includes(
          r.attendance_status,
        )
      )
        emp.total_absent++;
    ...
    return (
      <td className="px-2 py-2 text-center">
        <div className="flex flex-col items-center gap-1">
          <span className="w-full rounded text-xs font-medium bg-gray-100 text-gray-400 dark:bg-gray-800 dark:text-gray-600 px-1 py-0.5">
            —
          </span>
          <span className="w-full rounded text-xs font-medium bg-gray-100 text-gray-400 dark:bg-gray-800 dark:text-gray-600 px-1 py-0.5">
            —
          </span>
          <span className="w-full rounded text-xs font-bold bg-gray-100 text-gray-400 dark:bg-gray-800 dark:text-gray-600 px-1 py-0.5">
            —
          </span>
        </div>
      </td>
    ...
    <td className="px-2 py-2">
      <div className="flex flex-col gap-1 min-w-[80px]">
        ...
        <div className="flex items-center gap-1">
          ...
          <span
            className={`flex-1 rounded text-[11px] font-semibold text-center py-0.5 ${entradaColor}`}
          >
        ...
        <div className="flex items-center gap-1">
          ...
          <span
            className={`flex-1 rounded text-[11px] font-semibold text-center py-0.5 ${salidaColor}`}
          >
        ...
        <div className="flex items-center gap-1">
          ...
          <span
            className={`flex-1 rounded text-[11px] font-bold text-center py-0.5 ${horasColor}`}
          >
            {fmtMinutos(worked_minutes)}
            {has_overtime && (
              <span className="ml-1 text-[9px] opacity-80">
                +{fmtMinutos(overtime_minutes)}
              </span>
            )}
          </span>
        </div>
...
export default function AttendancePage() {
  ...
  const [lunesSemana, setLunesSemana] = useState(() =>
    getLunesDeUnaSemana(new Date()),
  );
  const [busqueda, setBusqueda] = useState("");
  const [filtroDept, setFiltroDept] = useState("");
  const [perPage, setPerPage] = useState(10);
  const [pagina, setPagina] = useState(1);
  const [drawerExportacionAbierto, setDrawerExportacionAbierto] =
    useState(false);
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useWeeklyAttendance(dateFrom, dateTo);
  ...
  const empleados = useMemo(() => {
    ...
    return lista.filter((e) => {
      const matchBusq =
        ...
        e.employee_number.includes(busqueda);
      const matchDept =
        !filtroDept ||
        e.department.toLowerCase().includes(filtroDept.toLowerCase());
      return matchBusq && matchDept;
    });
  }, [rawData, diasSemana, busqueda, filtroDept]);

  const totalPaginas = Math.ceil(empleados.length / perPage);
  const empleadosPag = empleados.slice(
    (pagina - 1) * perPage,
    pagina * perPage,
  );
  ...
  const irSemanaActual = () => {
    setLunesSemana(getLunesDeUnaSemana(new Date()));
    setPagina(1);
  };
  ...
  const exportarExcelPorRango = async ({
    ...
    try {
      ...
      parametros.append("from", fechaInicio);
      parametros.append("to", fechaFin);

      const respuesta = await api.get(
        `/attendance/export?${parametros.toString()}`,
        {
          responseType: "blob",
        },
      );
      ...
      enlace.setAttribute(
        "download",
        `Zenda_by_SISU_Checadas_${fechaInicio}_${fechaFin}.xlsx`,
      );
      ...
      setDrawerExportacionAbierto(false);
    } catch (error) {
      console.error("Error al exportar checadas", error);
    } finally {
      setExportando(false);
    }
  };
  ...
  const btnNav =
    "h-8 w-8 rounded-lg border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition text-sm font-bold";

  return (
    <>
      <PageMeta
        title="Asistencia"
        description="Control semanal de asistencia"
      />
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-4 sm:p-6 space-y-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <Link
            to="/asistencia/relojes"
            className="inline-flex items-center rounded-xl border border-[#465fff]/20 bg-[#465fff]/10 px-4 py-2 text-sm font-bold text-[#465fff] hover:bg-[#465fff]/15"
          >
            Relojes checadores
          </Link>
          ...
          <div className="flex items-center gap-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2 shadow-sm">
            <button onClick={irSemanaAnterior} className={btnNav}>
              ‹
            </button>
            ...
            <button onClick={irSemanaSiguiente} className={btnNav}>
              ›
            </button>
            <button
              onClick={irSemanaActual}
              className="ml-1 h-8 rounded-lg bg-brand-500 px-3 text-xs font-semibold text-white hover:bg-brand-600 transition"
            >
              Hoy
            </button>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            ...
            {
              label: "Asistencias",
              val: totales.presentes,
              color: "text-emerald-600 dark:text-emerald-400",
              bg: "bg-emerald-50 dark:bg-emerald-900/20",
            },
            {
              label: "Retardos",
              val: totales.retardos,
            ...
            <div
              key={label}
              className={`${bg} rounded-xl border border-slate-200 dark:border-slate-800 px-4 py-3 shadow-sm`}
            >
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-3 shadow-sm">
          ...
          <div className="ml-auto flex flex-wrap items-center justify-end gap-2">
            ...
            >
              <span className={isFetching ? "inline-block animate-spin" : ""}>
                ↻
              </span>
            ...
            {esRh && (
              <button
                type="button"
                onClick={() => setDrawerExportacionAbierto(true)}
                disabled={exportando}
                  ...
                  {diasSemana.map((fecha, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          className={`text-[10px] font-normal mt-0.5 ${esHoy ? "text-brand-500" : "text-slate-400"}`}
                        >
                          {String(d.getDate()).padStart(2, "0")}/
                          {MESES[d.getMonth()]}
                          {esHoy && (
                            <span className="ml-1 rounded-full bg-brand-500 px-1 text-white text-[9px]">
                              HOY
                            </span>
                          )}
                        </div>
          ...
          {totalPaginas > 1 && (
            <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-800 px-4 py-3">
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Mostrando {(pagina - 1) * perPage + 1}–
                {Math.min(pagina * perPage, empleados.length)} de{" "}
                {empleados.length}
              </p>
      ...
      <AttendanceExportDrawer
        abierto={drawerExportacionAbierto}
        onClose={() => setDrawerExportacionAbierto(false)}
        exportando={exportando}
        valoresIniciales={{
          fechaInicio: dateFrom,
          fechaFin: dateTo,
        }}
        onExportar={exportarExcelPorRango}
      />
    </>
  );
}

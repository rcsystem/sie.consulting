...
import type {
  Departamento,
  ErrorImportacionDepartamento,
  RespuestaImportacionDepartamento,
  RespuestaPaginadaDepartamentos,
} from "../../types/departments";

export default function DepartmentsPage() {
  const [departamentos, setDepartamentos] = useState<Departamento[]>([]);
  const [catalogoPadres, setCatalogoPadres] = useState<Departamento[]>([]);
  ...
  const [resultadoImportacion, setResultadoImportacion] =
    useState<RespuestaImportacionDepartamento | null>(null);
  ...
  const cargarCatalogoPadres = async () => {
    try {
      const response = await api.get<Departamento[]>("/catalogs/departments");
      setCatalogoPadres(response.data);
    } catch (error) {
      console.error(
  ...
  const cargarDepartamentos = async (
    ...
    try {
      const { data } = await api.get<RespuestaPaginadaDepartamentos>(
        "/departments",
        {
          params: {
            search: busqueda || undefined,
            is_active: filtroActivo !== "" ? filtroActivo : undefined,
            parent_id: filtroPadre || undefined,
            page: pagina,
            per_page: limite,
          },
        },
  ...
  useEffect(() => {
    cargarDepartamentos(1, porPagina);
  }, []);

  const buscar = async (e: React.FormEvent) => {
    e.preventDefault();
    await cargarDepartamentos(1, porPagina);
  };
  ...
  const abrirEditar = async (departamento: Departamento) => {
    try {
      const response = await api.get(`/departments/${departamento.id}`, {
        headers: {
          Accept: "application/json",
        },
      });

      setDepartamentoEditar(response.data.department);
      setModalAbierto(true);
    } catch (error) {
      console.error(
        "No fue posible cargar el detalle del departamento.",
        error,
      );
      alert("No fue posible abrir el departamento para edición.");
    }
  };
  ...
  const cambiarEstatus = async (departamento: Departamento) => {
    await api.patch(`/departments/${departamento.id}/toggle-status`);
    await cargarDepartamentos(paginaActual, porPagina);
    await cargarCatalogoPadres();
  };
  ...
  const descargarPlantilla = async () => {
    try {
      ...
      enlace.href = url;
      enlace.setAttribute("download", "plantilla_departamentos.xlsx");
  ...
  const subirArchivo = async () => {
    ...
    try {
      ...
      const { data } = await api.post<RespuestaImportacionDepartamento>(
        "/departments/import",
        formData,
        {
          headers: {
  ...
  return (
    <>
      <PageMeta
        title="Departamentos | SIE"
        description="Administración del catálogo de departamentos"
      />
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-3 xl:justify-end">
              ...
              >
                {subiendo ? "Procesando..." : "Subir departamentos"}
              </button>
              ...
              {resultadoImportacion.errores.length > 0 ? (
                <div className="mt-4 space-y-3">
                  {resultadoImportacion.errores.map(
                    (item: ErrorImportacionDepartamento, index) => (
                      <div
          ...
          >
            ...
            >
              ...
              {catalogoPadres.map((departamento) => (
              ...
              <tbody>
                {cargando ? (
                  ...
                  departamentos.map((departamento) => (
                    ...
                    >
                      <td className="px-4 py-4">{departamento.code || "-"}</td>
                      <td className="px-4 py-4">
                        <div>
                          ...
                          <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                            {departamento.description || "Sin descripción"}
                          </p>
                        </div>
                      </td>
                      <td className="px-4 py-4">
                        {departamento.parent?.name || "Sin padre"}
                      </td>
                      <td className="px-4 py-4">
                        {departamento.positions_count ?? 0}
                      </td>
                      <td className="px-4 py-4">
                      ...
                      <td className="px-4 py-4">
                        ...
                        >
                          {departamento.is_active ? "Activo" : "Inactivo"}
                        </span>
                      </td>
                      <td className="px-4 py-4">
                        <div className="flex flex-wrap gap-2">
                          <button
                            type="button"
                            onClick={() => abrirEditar(departamento)}
                            className="inline-flex h-10 items-center justify-center  border border-gray-300 px-4 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200"
                          >
                            Editar
                          </button>
                          ...
                          <button
                            type="button"
                            onClick={() => cambiarEstatus(departamento)}
                            className={`inline-flex h-10 items-center justify-center border border-gray-300 px-4 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 `}
                          >
                            {departamento.is_active ? "Desactivar" : "Activar"}
                          </button>
                        </div>
                      </td>
                    </tr>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <select
                ...
                onChange={async (e) => {
                  ...
                  await cargarDepartamentos(1, nuevoLimite);
                }}
              ...
              <button
                type="button"
                onClick={() => cargarDepartamentos(paginaActual - 1, porPagina)}
      ...
      <DepartmentFormModal
        ...
        departamentosPadre={catalogoPadres}
      />
    </>
  );
}

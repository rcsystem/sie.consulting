...
import type {
  ErrorImportacionPuesto,
  Puesto,
  RespuestaImportacionPuesto,
  RespuestaPaginadaPuestos,
...
export default function PositionsPage() {
  ...
  const [resultadoImportacion, setResultadoImportacion] =
    useState<RespuestaImportacionPuesto | null>(null);
  ...
  const cargarDepartamentos = async () => {
    try {
      const response = await api.get<CatalogoBase[]>("/catalogs/departments");
  ...
  const cargarPuestos = async (pagina = paginaActual, limite = porPagina) => {
    setCargando(true);

    try {
      const { data } = await api.get<RespuestaPaginadaPuestos>("/positions", {
        params: {
      ...
      console.error("No fue posible cargar los puestos.", error);
    } finally {
      setCargando(false);
    }
  };
  ...
  useEffect(() => {
    cargarPuestos(1, porPagina);
  }, []);

  const buscar = async (e: React.FormEvent) => {
    e.preventDefault();
    await cargarPuestos(1, porPagina);
  };
  ...
  const abrirEditar = async (puesto: Puesto) => {
    try {
      const response = await api.get(`/positions/${puesto.id}`, {
        headers: {
          Accept: "application/json",
        },
      });

      setPuestoEditar(response.data.position);
      setModalAbierto(true);
    } catch (error) {
      console.error("No fue posible cargar el detalle del puesto.", error);
      alert("No fue posible abrir el puesto para edición.");
    }
  };
  ...
  const cambiarEstatus = async (puesto: Puesto) => {
    await api.patch(`/positions/${puesto.id}/toggle-status`);
    await cargarPuestos(paginaActual, porPagina);
  };
  ...
  const descargarPlantilla = async () => {
    try {
      ...
      const response = await api.get("/positions/import/template", {
        responseType: "blob",
        headers: {
          Accept: "application/json",
        },
      ...
      enlace.href = url;
      enlace.setAttribute("download", "plantilla_puestos.xlsx");
  ...
  const subirArchivo = async () => {
    ...
    try {
      ...
      const { data } = await api.post<RespuestaImportacionPuesto>(
        "/positions/import",
        formData,
        {
          headers: {
  ...
  return (
    <>
      <PageMeta
        title="Puestos | SIE"
        description="Administración del catálogo de puestos"
      />
          ...
          <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-2 max-w-2xl text-sm text-slate-500 dark:text-slate-400">
                Alta, edición, activación e importación del catálogo de puestos.
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 xl:justify-end">
              ...
              >
                {subiendo ? "Procesando..." : "Subir puestos"}
              </button>
              ...
              {resultadoImportacion.errores.length > 0 ? (
                <div className="mt-4 space-y-3">
                  {resultadoImportacion.errores.map(
                    (item: ErrorImportacionPuesto, index) => (
                      <div
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
              ...
              <tbody>
                {cargando ? (
                  ...
                  puestos.map((puesto) => (
                    ...
                    >
                      <td className="px-4 py-4">{puesto.code || "-"}</td>
                      ...
                      <td className="px-4 py-4">
                        {puesto.department?.name || "-"}
                      </td>
                      <td className="px-4 py-4">
                        {puesto.base_salary != null
                          ? Number(puesto.base_salary).toLocaleString("es-MX", {
                              style: "currency",
                              currency: "MXN",
                            })
                          : "-"}
                      </td>
                      <td className="px-4 py-4">{puesto.users_count ?? 0}</td>
                      <td className="px-4 py-4">
                        <span
                        ...
                        >
                          {puesto.is_active ? "Activo" : "Inactivo"}
                        </span>
                      </td>
                      <td className="px-4 py-4">
                        <div className="flex flex-wrap gap-2">
                          ...
                          >
                            {puesto.is_active ? "Desactivar" : "Activar"}
                          </button>
                        </div>
                      </td>
                    </tr>
          ...
          <div className="mt-4 flex flex-col gap-3 border-t border-gray-200 pt-4 text-sm text-gray-600 dark:border-gray-800 dark:text-gray-400 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button
                type="button"
                onClick={() => cargarPuestos(paginaActual - 1, porPagina)}

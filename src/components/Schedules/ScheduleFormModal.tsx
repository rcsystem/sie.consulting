...
import type { FormularioHorario, Horario } from "../../types/schedules";
...
const estadoInicial: FormularioHorario = {
  name: "",
  entry_time: "",
  exit_time: "",
  entry_tolerance_minutes: "0",
...
export default function ScheduleFormModal({
  ...
  const [formulario, setFormulario] = useState<FormularioHorario>(estadoInicial);
  ...
  useEffect(() => {
    ...
    if (horarioEditar) {
      setFormulario({
        name: horarioEditar.name ?? "",
        entry_time: horarioEditar.entry_time ?? "",
        exit_time: horarioEditar.exit_time ?? "",
        entry_tolerance_minutes:
          horarioEditar.entry_tolerance_minutes != null
            ? String(horarioEditar.entry_tolerance_minutes)
            : "0",
        exit_tolerance_minutes:
          horarioEditar.exit_tolerance_minutes != null
            ? String(horarioEditar.exit_tolerance_minutes)
            : "0",
        working_days: horarioEditar.working_days ?? [],
        description: horarioEditar.description ?? "",
        is_active: horarioEditar.is_active,
      });
    } else {
    ...
    setError("");
    setErroresLista([]);
  }, [abierto, horarioEditar]);

  const actualizarCampo = (
    campo: keyof FormularioHorario,
  ...
  const alternarDia = (dia: string) => {
    const existe = formulario.working_days.includes(dia);

    if (existe) {
      actualizarCampo(
        "working_days",
        formulario.working_days.filter((item) => item !== dia),
      );
      return;
    }

    actualizarCampo("working_days", [...formulario.working_days, dia]);
  };
  ...
  const guardar = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        entry_time: formulario.entry_time,
        exit_time: formulario.exit_time,
        entry_tolerance_minutes:
          formulario.entry_tolerance_minutes.trim() !== ""
            ? Number(formulario.entry_tolerance_minutes)
            : 0,
        exit_tolerance_minutes:
          formulario.exit_tolerance_minutes.trim() !== ""
            ? Number(formulario.exit_tolerance_minutes)
            : 0,
        working_days: formulario.working_days,
        description: formulario.description.trim() || null,
        is_active: formulario.is_active,
      };

      if (horarioEditar) {
        await api.put(`/schedules/${horarioEditar.id}`, payload);
      } else {
        ...
        <div className="flex items-center justify-between border-b border-gray-200 bg-brand-500 px-6 py-4 text-white">
          <div>
            <h2 className="text-xl font-semibold">
              {horarioEditar ? "Editar horario" : "Nuevo horario"}
            </h2>
            ...
            <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
              ...
              <div>
                ...
                <input
                  ...
                  min="0"
                  max="180"
                  value={formulario.entry_tolerance_minutes}
                  onChange={(e) =>
              ...
              <div>
                ...
                <input
                  ...
                  min="0"
                  max="180"
                  value={formulario.exit_tolerance_minutes}
                  onChange={(e) =>
              ...
              <div className="md:col-span-2">
                ...
                <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                  {diasDisponibles.map((dia) => {
                    const activo = formulario.working_days.includes(dia.value);
